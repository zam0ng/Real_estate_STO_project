            ...
            {fromRecent.map((item,index)=>{
                return(
                    <div className='w-full h-[30%] flex flex-col items-center text-sm mt-2 mb-2' key={index}>
                        <div className={`w-[80%] h-1/5 text-xs md:text-lg flex justify-between ${item.order_type === "판매" ? "blueText" : "redText"}`}>
                            <div className='flex justify-center items-center'>{item.order_type}</div>
