...
const DailyHistoryTable: React.FC = () => {
  ...
  const fetchDailyHistory = async (): Promise<DailyHistoryRequest[]> => {
    const response = await fetch(`http://127.0.0.1:8080/market/detail/daliyQuote/${selectedPropertyName}`);
  ...
  return (
    <div className='w-[90%] h-full '>
      <div className='w-full h-[10%]'>
        <TableHeader header1='날짜' header2='익일기준가' header3='변동액' header4='거래대금' />
      </div>
      <div className='w-full h-[90%] flex flex-col'>
        {data && data.map((item,index)=>(
          <DailyHistoryTableInfo key={index} date={item.date} next_trade_price={item.next_trade_price} 
          fluctuation_price={item.fluctuation_price} total_price={item.total_price} />
        ))}
      </div>
    </div>
  )
