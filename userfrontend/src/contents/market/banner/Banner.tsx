...
const Banner: React.FC = () => {
  ...
  return (
    <>
      {isLoading ? <div>Loading...</div> : dataRep &&
      <div className='w-[80%] h-4/5 bg-slate-100 mt-10 rounded-lg flex flex-col items-center justify-around 
      shadow-lg'>
        <BuildingImg img={dataRep.subscription_img} totalPrice={dataRep.subscription_totalprice} />
        <BuildingInfo name={dataRep.subscription_name} description={dataRep.subscription_description} completionRate={completionRate} restdate={dataRep.subscription_restdate} />
        <BuyBtn />
      </div>}
    </>
  )
