...
export const tradingVolumeRN = async(req : Request , res : Response) =>{
    ...
    try {
        const result = await Trades.findAll({
            ...
            raw : true,
            having: sequelize.where(sequelize.fn('DATE_TRUNC', sequelize.literal(`'day'`), sequelize.col('createdAt')), '=', currentDate),
        })

        const estateNames = result.map(item => item.real_estate_name);

        const findOneResults = await Promise.all(estateNames.map(async (name) => {
...
export const rateOfReturn = async(req : Request , res : Response) =>{
    ...
    try {
        ...
        const findOneResult = await Promise.all(estateName.map(async(name)=>{
