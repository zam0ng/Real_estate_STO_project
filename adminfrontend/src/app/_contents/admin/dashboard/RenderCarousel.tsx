...
const RenderCarousel: React.FC<CurrentSituationDataProps> = ( {currentSituationData} ) => {
    ...
    const fileNameArr = replacedImgPathsArr.map((item : string) => item.split('/')[2])
    const finalDomain = `${process.env.NEXT_PUBLIC_API_URL || process.env.NEXT_PUBLIC_PROD_URL}`
    const finalImageURLArr = fileNameArr.map( ( item : string) => `${finalDomain}/estate_img/${item}` )
    ...
    const prevSlide = ()=>{
        ...
        const newIndex = isFirstSlide? finalImageURLArr.length - 1 : currentIndex - 1 ;
        setTimeout( () => {
          setCurrentIndex(newIndex)
          setAnimation("fadeIn");
        } , 1000)
  ...
  return (
    ...
    <div className="relative flex items-center h-64 rounded-2xl w-30rem justify-evenly ">
      ...
      <div className="h-11.5rem w-16.25rem   flex flex-col">
            ...
            <div className="flex w-full h-full mb-1">
                  ...
                  <div className="z-10 flex flex-col items-center justify-end w-1/3 h-auto top-20 ">
                    <p className="text-dashboard_carousel_black text-1.687rem font-extrabold">{tokenPriceArr[currentIndex] != null? tokenPriceArr[currentIndex]  : 0}₩ </p>
      ...
      <div className="absolute w-3/4 h-1 m-5 mx-auto bg-gray-300 rounded-full -bottom-3 left-14">
          <div className="absolute h-1 bg-gray-500 rounded-full" style={{
